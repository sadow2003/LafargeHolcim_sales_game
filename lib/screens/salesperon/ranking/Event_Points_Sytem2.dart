...
class EventLeaderBoardBody extends StatelessWidget{
  ...
  Widget _congratsOverlay(){
    final savedWinner = (resultData!['winners'] as List<dynamic>?)
        ?.cast<Map<String,dynamic>>();

    if(savedWinner != null && savedWinner.isNotEmpty){
      return CongratulationOverlay(
        winners: null,
        resultWinners: savedWinner,
        rewards: rewards,
    ...
    final actualWinners =docs
        .where((d)=>
        (((d.data() as Map<String,dynamic>)['totalPonits'] as num ?)
              ?.toInt() ??
              0)>
            0)
          .take(3)
