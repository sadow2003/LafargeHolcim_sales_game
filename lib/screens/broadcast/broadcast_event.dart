...
/// A richer, themed card for broadcasts of type 'event' — used both for the
/// automatic start/end announcements posted by [EventManagementService] and
/// for any manually-posted 'Event' broadcast.
class EventBroadcastCard extends StatelessWidget {
  final String docId;
  final Map<String, dynamic> data;
  final String? currentUid;
  final String currentUserRole;

  const EventBroadcastCard({
    super.key,
    required this.docId,
    required this.data,
  ...
  Widget build(BuildContext context) {
    ...
    // The service posts content as "headline\nbody...". Fall back gracefully
    // for manually-typed event broadcasts that don't follow that shape.
    final lines    = content.split('\n');
    final headline = lines.first;
    var   body     = lines.skip(1).join('\n').trim();

    // The chip below already surfaces the category/product, so strip the
    // redundant "Category: X — " / "Product: X — " prefix from the body.
    ...
    // Fall back to sniffing the emoji for broadcasts posted before the
    // eventPhase field existed.
    final isEnded = phase != null ? phase == 'ended' : headline.contains('🏁');

    // Light blue announces a new event; red marks it closing.
